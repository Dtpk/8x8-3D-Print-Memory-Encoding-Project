...
GBA version will get a 3D printer option it will let you set the temps home then print a already made square over otg to the 3D printer in my case the ender 3 this is all just for laughs but wanted to try and print over link cable connected to a pico to the machine no promises this build will get done and released but will do my best. Through bit banging pin 3 and some buffer tricks on the pico I got the first 3D print its so close so expect this to be avaliable soon with some a 3D print model for a stand. Right now only marlin firmware and ender 3 build plates are set but any printer with marlin this should work with it when it goes live.
